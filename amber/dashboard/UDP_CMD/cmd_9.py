...
async def gripperCtrl(action, force):


    s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)                # Standard socket processes
    s.bind(("0.0.0.0", 12329))
   #payloadS = robot_joint_position(7, 12, 0, 8)          # Fill struct for send with numbers
   #s.sendto(payloadS, (IP_ADDR, 25001))                                # Default port is 25001
    payloadS = robot_joint_position(9, 13, 114514,action,force,0)               # 0 : open, 1 : close
    s.sendto(payloadS, (IP_ADDR, 26001))     
